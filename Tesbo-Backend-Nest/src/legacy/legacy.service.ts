  ...
  async listCycles(projectId: string) {
    const res = await this.db.query(
      ...
      `SELECT c.*,
              ...
              -- Retest counts as untested, not as its own bucket: a case flagged for retest has no
              -- settled result for this run. Leaving it out of every bucket made these counters sum
              -- to less than total_cases. Filtered off e.id like the buckets above, so a cycle_item
              -- with no live execution stays out of both the numerator and total_cases.
              COUNT(e.id) FILTER (WHERE e.status IS NULL OR e.status IN ('Untested', 'Retest'))::int AS untested
       FROM cycles c
       LEFT JOIN cycle_items ci ON ci.cycle_id = c.id
