...
pipeline {
  ...
  stages {
    ...
    stage('Deploy prod (blue/green)') {
      steps {
        // Remote script via bash -s. Unquoted heredoc: Jenkins env (${DEPLOY_PATH} etc.) expands
        // on the agent; remote shell vars are escaped as \\$ so they run on the prod host.
        sh '''
          set -eu
          ...
          ssh $SSH_OPTS "${DEPLOY_USER}@${DEPLOY_HOST}" bash -s <<ENDSSH
          set -eu
          cd ${DEPLOY_PATH}
